...
export default function Navbar({ refs }: NavbarProps) {
  ...
  return (
    <>
      {/* ── TOP NAVBAR (desktop full / mobile simplified header) ── */}
      <nav
        ...
        <div className="flex justify-between items-center max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-3">
          ...
          <div className="flex items-center gap-3">
            <Image
              ...
              priority
            />
            {/* Hide company name on mobile to keep header clean */}
            <span
          ...
          {/* Mobile: "Find a Ride" CTA in header (replaces hamburger) */}
          <div className="md:hidden">
            <Link href="/#search">
              <button
                className="font-semibold px-4 py-2 rounded-full text-sm text-white"
                style={{ background: "#2F5EEA" }}
              >
                Find a Ride
              </button>
      ...
      {/* ── MOBILE BOTTOM PILL NAV (Puri Escapes style) ── */}
      <div className="fixed inset-x-0 bottom-4 z-50 flex justify-center md:hidden">
        <div
          ...
          style={{
            ...
            boxShadow: "0 18px 45px rgba(0,0,0,0.18), 0 4px 12px rgba(47,94,234,0.12)",
          }}
        >
          {navItems.map((item, idx) => {
            const Icon = item.icon;
